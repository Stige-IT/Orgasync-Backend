...
@address_auth_router.get(
    "", status_code=status.HTTP_200_OK, response_model=AddressResponse
)
...
async def update_address(
    request: Request, data: AddressRequest, db: Session = Depends(get_db)
):
    id_address = request.user.id_address
    address = db.query(Address).get(id_address)
    if not address:
        uuidFormat = str(uuid.uuid4())
        print(uuidFormat)
        user = db.query(UserModel).get(request.user.id)
        user.id_address = uuidFormat
        new_address = Address(
            id=uuidFormat,
            street=data.street,
...
# get province
@address_router.get(
    "/province", status_code=status.HTTP_200_OK, response_model=List[LocationResponse]
)
...
async def get_regency(province_id: int, db: Session = Depends(get_db)):
    regency = (
        db.query(AddressRegency).filter(AddressRegency.province_id == province_id).all()
    )
    return regency
...
async def get_district(regency_id: int, db: Session = Depends(get_db)):
    district = (
        db.query(AddressDistrict).filter(AddressDistrict.regency_id == regency_id).all()
    )
    return district
...
async def get_village(district_id: int, db: Session = Depends(get_db)):
    village = (
        db.query(AddressVillage).filter(AddressVillage.district_id == district_id).all()
    )
    return village
...
# get country
@address_router.get(
    "/country", status_code=status.HTTP_200_OK, response_model=List[LocationResponse]
)
